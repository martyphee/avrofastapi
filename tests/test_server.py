...
def format_request(
    handshake: HandshakeRequest = None,
...
class TestAvroServer:
    ...
    @pytest.mark.parametrize(
        ...
        [
            None,
            avro_frame(
                handshake_serializer(
                    HandshakeRequest(
                        clientHash=b"deadbeefdeadbeef", serverHash=b"deadbeefdeadbeef"
                    )
                )
            ),
    ...
    def test_AvroRoute_AllAvroHeadersInvalidHandshake_ReturnsAvroHandshake(
        self, payload: bytes
    ):
        # arrange
        app = AvroFastAPI()
        ...
        assert {get_name(Error), get_name(ValidationError)} == set(
            server_protocol["messages"]["test_func__post"].pop("errors")
        )
    ...
    def test_AvroRoute_AllAvroHeadersValidHandshakeNoBody_ReturnsHandshakeNoResponse(
        self,
    ):
        # arrange
    ...
    def test_AvroRoute_AllAvroHeadersValidHandshakeHandshakeCached_ReturnsHandshakeNoResponse(
        self,
    ):
        # arrange
        protocol = AvroProtocol(
    ...
    def test_AvroRoute_AllAvroHeadersCachedNullResponse_ReturnsHandshakeNoResponse(
        self,
    ):
        # arrange
        protocol = AvroProtocol(
    ...
    def test_AvroRoute_AllAvroHeadersInvalidRequest_ReturnsHandshakeAndError(self):
        ...
        response = client.post(
            ...
            content=format_request(
                handshake=handshake,
                request=RequestModel(A="1", B=-2, C=3.1),
                message="test_func__post",
            ),
        )
