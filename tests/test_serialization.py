...
@pytest.mark.parametrize(
    ...
    [
        ...
        NestedModelBasicTypes(
            A=BasicModelBaseTypes(A="string", B=1, C=1.1, D=b"abc", E=True), B=2
        ),
        BasicModelTypingTypes(A=[1], B={"a": 2}, C=None, D=3),
        BasicModelTypingTypes(A=[1], B={"a": 2}, C=None, D="3"),
        BasicModelCustomTypes(A=123, B=34.5),
        NestedModelUnionRecords(
            A=BasicModelAdvancedTypes(
                A=datetime.now(timezone.utc),
                B="abcde12345",
                C=Decimal("12.345"),
                D=BasicEnum.test2,
...
def test_serialize_ValidInput_ModelEncodedAndDecodedSuccessfully(
