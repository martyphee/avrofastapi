...
def _validate_avro_name(name: str):
    if _avro_name_format.match(name) is None:
        raise AvroException(
            f"{name} does not match the avro name format: names must start with [A-Za-z_] and subsequently contain only [A-Za-z0-9_]"
        )


def get_name(model: Type) -> str:
    origin: Optional[Type] = getattr(
        model, "__origin__", None
    )  # for types from typing library
    name: str = None
...
def _validate_avro_namespace(namespace: str, parent_namespace: str = None):
    if not all(map(_avro_name_format.match, namespace.split("."))):
        raise AvroException(
            f"{namespace} does not match the avro namespace format: A namespace is a dot-separated sequence of names. names must start with [A-Za-z_] and subsequently contain only [A-Za-z0-9_]"
        )
...
class AvroSchemaGenerator:
    ...
    def _convert_enum(
        ...
        values: Optional[List[str]] = list(
            map(lambda x: x.value, model.__members__.values())
        )

        if len(values) != len(set(values)):
            raise AvroException(
                "enums must contain all unique values to be avro encoded"
            )
    ...
    def _convert_map(
        ...
        if not hasattr(model, "__args__"):
            raise AvroException(
                "typing.Dict must be used to determine key/value type, not dict"
            )
