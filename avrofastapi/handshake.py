...
class AvroMessage(BaseModel):
    ...
    def validate_models_exist(cls, value, values):
        ...
        assert (
            not missing_types
        ), f'string types must exist in types, [{", ".join(missing_types)}] missing from types'
        return value
    ...
    def validate_response_exists(cls, value, values):
        if isinstance(value, str) and value not in __built_in_types__:
            assert value in {
                v["name"] for v in values.get("types", [])
            }, f"string types must exist in types, {value} missing from types"
        return value
