...
output: Optional[bytes] = b"".join(
    Popen(
        ["git", "config", "--get", "remote.origin.url"], stdout=PIPE, stderr=PIPE
    ).communicate()
)
if output and not output.startswith(b"fatal"):
    ...
    output = b"".join(
        Popen(
            ["git", "rev-parse", "--show-toplevel"], stdout=PIPE, stderr=PIPE
        ).communicate()
    )
    if output and not output.startswith(b"fatal"):
...
output = b"".join(
    Popen(["git", "rev-parse", "HEAD"], stdout=PIPE, stderr=PIPE).communicate()
)
if output and not output.startswith(b"fatal"):
