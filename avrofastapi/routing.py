...
from avrofastapi.handshake import (
    MD5,
    AvroMessage,
    AvroProtocol,
    CallRequest,
...
class AvroJsonResponse(Response):
    ...
    def __init__(
            self: "AvroJsonResponse",
            serializable_body: dict = None,
            model: BaseModel = None,
            *args: Tuple[Any],
            serializer: AvroSerializer = None,
            handshake: HandshakeResponse = None,
            error: bool = False,
            **kwargs: Dict[str, Any],
    ):
    ...
    async def __call__(
            self: "AvroJsonResponse", scope: Scope, receive: Receive, send: Send
    ):
        ...
        if "avro/binary" in request.headers.get("accept"):
            ...
            handshake: HandshakeResponse = (
                request.scope["avro_handshake"]
                if "avro_handshake" in request.scope
                else self._handshake
            )
            serializer: AvroSerializer = (
                    self._serializer or self.__writer_cache__[type(self._model)]
            )

            if handshake and not (
                    # check to see if they are using header protocol and we can skip returning handshake
                    handshake.match == HandshakeMatch.both
                    and "avro-server-hash" in request.headers
                    and "avro-client-hash" in request.headers
                    and "avro-handshake" in request.headers
                    and not request.headers["avro-handshake"] == "true"
            ):
                if self._model:
                    logger.debug(
                        "response: handshake.%s + model: %s",
                        handshake.match.name,
                    ...
                    self.body = (
                        ...
                        call_serializer(
                            CallResponse(
                                error=self._error, response=serializer(self._model)
                            ),
                        )
                    )
                            + avro_frame()
                    ...
                    self.body = (
                            avro_frame(handshake_serializer(handshake)) + avro_frame()
                    )
                ...
                raise ValueError(
                    "at least a handshake or model is required to return an avro response"
                )
            ...
            self.headers.update(
                {
                    ...
                    "avro-handshake-match": (
                        handshake.match.value
                        if handshake
                        else HandshakeMatch.none.value
                    ),
                }
            )
...
class AvroRoute(APIRoute):
    ...
    def __init__(
        ...
        if self.response_model is None and self.status_code not in {204}:
            warn(
                "in order for the avro handshake to be performed, a response model must be passed or the status code set to 204"
            )

        if self.response_model:
            self.response_schema = convert_schema(self.response_model)

    async def handle_avro(
            self: "AvroRoute", scope: Scope, receive: Receive, send: Send
    ) -> None:
    ...
    def get_route_handler(self: "AvroRoute") -> ASGIApp:
        ...
        response_model_include: Optional[Union[SetIntStr, DictIntStrAny]] = (
            self.response_model_include
        )
        response_model_exclude: Optional[Union[SetIntStr, DictIntStrAny]] = (
        ...
        async def app(request: Request) -> Response:
            ...
            try:
                ...
                protocol_hash: bytes
                serializer: AvroSerializer
                server_protocol, protocol_hash, serializer = request.scope[
                    "router"
                ]._server_protocol
            ...
            if errors:
                ...
                error = ValidationError(
                    detail=[ValidationErrorDetail(**e) for e in errors[0].exc.errors()]
                )
                return AvroJsonResponse(
                    model=error,
                    serializer=serializer,
                ...
                response = actual_response_class(
                    serializable_body=response_data, model=raw_response, **response_args
                )
...
class AvroRouter(APIRouter):
    ...
    def __init__(
        ...
        self._server_protocol: Tuple[str, MD5, AvroSerializer] = (
            protocol,
            md5(protocol.encode()).digest(),
            AvroSerializer(Union[Error, ValidationError]),
        )
        ...
        self._client_protocol_cache: Dict[MD5, Tuple[AvroDeserializer, bool]] = (
            OrderedDict()
        )
    ...
    def add_avro_route(self: "AvroRouter", route: APIRoute) -> None:
        if route.unique_id in self._avro_routes:
            warn(
                f"existing route found for route id {route.unique_id}. this route may be unreachable via avro: {self._avro_routes[route.unique_id]}"
            )
    ...
    def add_server_protocol(self: "AvroRouter", route: APIRoute) -> None:
        ...
        protocol.messages[route.unique_id] = AvroMessage(
            ...
            # optimize
            request=(
                convert_schema(route.body_field.type_)["fields"]
                if route.body_field
                else []
            ),
            # optimize
        ...
        self._server_protocol = (
            protocol_json,
            md5(protocol_json.encode()).digest(),
            AvroSerializer(errors),
        )

    async def check_schema_compatibility(
            self: "AvroRouter", handshake: HandshakeRequest
    ) -> Tuple[Dict[str, AvroDeserializer], bool]:
        """
        ...
        if not handshake.clientProtocol:
            raise AvroDecodeError(
                "client request protocol was not included and client request hash was not cached."
            )
        ...
        for route_id, client_message in client_protocol.messages.items():
            ...
            if route is None:
                raise AvroDecodeError(
                    f"route does not exist for client protocol message {route_id}."
                )
            ...
            if client_message.request:
                ...
                if route.body_field:
                    request_schema: AvroSchema = {
                        ...
                        "fields": [
                            (
                                {"type": client_protocol_types[r.pop("type")], **r}
                                if r["type"] in client_protocol_types
                                else r
                            )
                    ...
                    # optimize
                    request_compatibility: SchemaCompatibilityResult = (
                        AvroChecker.get_compatibility(
                            reader=parse(
                                json.dumps(convert_schema(route.body_field.type_))
                            ),
                    ...
                    if (
                            not request_compatibility.compatibility
                                == SchemaCompatibilityType.compatible
                    ):
                        raise AvroDecodeError(
                            "client request protocol is incompatible."
                        )

                    request_deserializers[route.unique_id] = AvroDeserializer(
                        route.body_field.type_,
                        route.body_schema,
                        client_protocol_request_schema,
                        parse=False,
                    )

                else:
                    raise AvroDecodeError(
                        "client protocol provided a request but route does not expect one."
                    )

            elif route.body_field:
                raise AvroDecodeError(
                    "client protocol did not provide a request but route expects one."
                )
            ...
            if response_schema:
                if route.response_model:
                    ...
                    response_compatibility: SchemaCompatibilityResult = (
                        AvroChecker.get_compatibility(
                    ...
                    client_compatible = (
                            client_compatible
                            and response_compatibility.compatibility
                            == SchemaCompatibilityType.compatible
                    )
        ...
        data = self._client_protocol_cache[handshake.clientHash] = (
        ...
        if len(self._client_protocol_cache) > self._client_protocol_max_size:
            ...
            async with self._cache_lock:
                ...
                for key in list(reversed(self._client_protocol_cache.keys()))[
                           len(self._client_protocol_cache) - self._client_protocol_max_size:
                           ]:
    ...
    async def settle_avro_handshake(
            self: "AvroRouter", request: Request
    ) -> Optional[APIRoute]:
        body = await request.body()

        if not body:
            raise AvroDecodeError(
                "no body was included with the avro request, a handshake must be provided with every request"
            )
        ...
        request_deserializers, response_compatibility = (
            await self.check_schema_compatibility(handshake_request)
        )
        ...
        if call_request.message not in self._avro_routes:
            raise AvroDecodeError(
                f'{call_request.message} not found in valid messages ({", ".join(self._avro_routes.keys())})'
            )
        ...
        if call_request.message not in request_deserializers:
            if route.body_field:
                raise AvroDecodeError(
                    f"call request message {call_request.message} was not found in client protocol"
                )
            ...
            request.scope["avro_body"] = request_deserializers[call_request.message](
    ...
    async def __call__(
            self: "AvroRouter", scope: Scope, receive: Receive, send: Send
    ) -> None:
        if "avro/binary" == Headers(scope=scope).get("content-type"):
            ...
            try:
                route: Optional[AvroRoute] = await self.settle_avro_handshake(
                ...
                logger.exception(
                    f'Something went wrong while decoding the request body. path: {scope["path"]}, refid: {refid.hex}'
                )
