        ...
        self._client_protocol: AvroProtocol = AvroProtocol(
            ...
            messages={
                self._message_name: AvroMessage(
                    request=(
                        convert_schema(request_model)["fields"] if request_model else []
                    ),
    ...
    async def __call__(
        ...
        **kwargs,
        ...
        handshake: HandshakeRequest = HandshakeRequest(
            ...
            clientProtocol=(
                self._client_protocol.json()
                if self._handshake_status is not HandshakeMatch.both
                else None
            ),
                ...
                **req,
                ...
                try:
                    ...
                    if response_handshake.match == HandshakeMatch.none:
                        ...
                        server_types = {
                            i["name"]: i
                            for i in server_protocol["messages"][self._message_name][
                                "types"
                            ]
                        }
                        server_response = server_protocol["messages"][
                            self._message_name
                        ]["response"]
                        ...
                        if self._response_model:
                            self._deserializer = AvroDeserializer(
                                ...
                                json.dumps(
                                    server_types[server_response]
                                    if server_response in server_types
                        ...
                        self._client_protocol.messages[self._message_name].response = (
                            server_response
                        )
                        self._client_hash: bytes = md5(
                            self._client_protocol.json().encode()
                        ).digest()
                    ...
                    if call_response.error:
                        raise ValueError(
                            f"error: {self._error_deserializer(call_response.response)}"
                        )
