...
class ABetterDatumWriter(DatumWriter):
    ...
    def write_enum(
        self,
        writers_schema: EnumSchema,
        datum: Union[Enum, str],
    ...
    def _writer_type_null_(
        writers_schema: Schema, datum: None, encoder: BinaryEncoder
    ) -> None:
        if datum is None:
            return encoder.write_null(datum)
        raise AvroTypeException(writers_schema, datum)

    def _writer_type_bool_(
        writers_schema: Schema, datum: bool, encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, bool):
            return encoder.write_boolean(datum)
        raise AvroTypeException(writers_schema, datum)

    def _writer_type_str_(
        writers_schema: Schema, datum: Union[str, UUID], encoder: BinaryEncoder
    ) -> None:
    ...
    def _writer_type_int_(
        writers_schema: Schema, datum: int, encoder: BinaryEncoder
    ) -> None:
    ...
    def _writer_type_float_(
        writers_schema: Schema, datum: Union[int, float], encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, (int, float)):
            return encoder.write_float(datum)
        raise AvroTypeException(writers_schema, datum)

    def _writer_type_double_(
        writers_schema: Schema, datum: Union[int, float], encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, (int, float)):
    ...
    def _writer_type_bytes_(
        writers_schema: Schema, datum: Union[Decimal, bytes], encoder: BinaryEncoder
    ) -> None:
        logical_type: Optional[Literal] = getattr(writers_schema, "logical_type", None)
        ...
        if logical_type == DECIMAL:
            ...
            if not (isinstance(scale, int) and scale > 0):
                warn(
                    IgnoredLogicalType(
                        f"Invalid decimal scale {scale}. Must be a positive integer."
                    )
                )
    ...
    def _writer_type_fixed_(
        self,
        writers_schema: Schema,
        datum: Union[Decimal, bytes],
        encoder: BinaryEncoder,
    ) -> None:
        logical_type: Optional[Literal] = getattr(writers_schema, "logical_type", None)
    ...
    def _writer_type_enum_(
        self, writers_schema: Schema, datum: Union[str, Enum], encoder: BinaryEncoder
    ) -> None:
    ...
    def _writer_type_array_(
        self, writers_schema: Schema, datum: Sequence, encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, Sequence):
            return self.write_array(writers_schema, datum, encoder)
        raise AvroTypeException(writers_schema, datum)

    def _writer_type_map_schema_(
        self, writers_schema: Schema, datum: Dict[str, Any], encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, Mapping):
            return self.write_map(writers_schema, datum, encoder)
        raise AvroTypeException(writers_schema, datum)

    def _writer_type_record_(
        self, writers_schema: Schema, datum: dict, encoder: BinaryEncoder
    ) -> None:
        if isinstance(datum, Mapping):
    ...
    def write_data(
        self, writers_schema: Schema, datum: Any, encoder: BinaryEncoder
    ) -> None:
        ...
        if writers_schema.type in self._writer_type_map_:
            return self._writer_type_map_[writers_schema.type](
        ...
        for cls in type(data).__mro__:
            if cls in _data_converter_map:
                self._writer.write_data(
                    self._writer.writers_schema, _data_converter_map[cls](data), encoder
                )
...
class AvroDeserializer:
    ...
    def __init__(
        ...
        read_schema: Union[Schema, str] = None,
        write_model: Union[Schema, Type[BaseModel], str] = None,
        parse: bool = True,
    ):
        assert read_model or (
            read_schema and parse == False
        ), "either read_model or read_schema must be provided. if only read schema is provided, parse must be false"
        write_schema: Schema
        ...
        if not read_schema:
            ...
            raise NotImplementedError(
                f'the type for read_schema "{type(read_schema)}" is not supported.'
            )
        ...
        if not write_model:
            ...
            raise NotImplementedError(
                f'the type for write_model "{type(write_model)}" is not supported.'
            )

        reader: DatumReader = DatumReader(write_schema, read_schema)

        if parse:
            self._parser: Callable[[bytes], read_model] = lambda x: parse_obj_as(
                read_model, reader.read(x)
            )
